...
export class ClaudeProvider extends BaseAIService {
  ...
  async chat(
    ...
    try {
      ...
      throw new Error(`Claude AI service error: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
3. Overall quality score (0-100)`;
  ...
  async getProviderStatus(): Promise<{ provider: string; status: 'online' | 'offline' | 'limited' }[]> {
    return [{ provider: 'claude', status: 'online' }];
  }

  // Private helper methods
  private formatContext(context: ProjectContext | any): string {
    const sections: string[] = [];
    
    // Project info
    sections.push(`Project: ${context.name || 'Unknown'}`);
    sections.push(`Framework: ${context.framework || 'Unknown'}`);
    sections.push(`Language: ${context.language || 'TypeScript'}`);
    
    // File structure with content
    if (context.fileStructure && context.fileStructure.length > 0) {
      sections.push('\n=== PROJECT FILES ===');
      
      const filesWithContent = context.fileStructure.filter((f: any) => f.type === 'file' && f.content);
      sections.push(`Total files: ${context.fileStructure.length}, Files with content: ${filesWithContent.length}`);
      
      // Include actual file contents for AI to analyze
      filesWithContent.forEach((file: any) => {
        sections.push(`\n--- File: ${file.path} ---`);
        sections.push(`\`\`\`${this.getLanguageFromPath(file.path)}`);
        sections.push(file.content.substring(0, 5000)); // Limit content per file
        sections.push('```');
      });
    }
    ...
    if (context.dependencies && Object.keys(context.dependencies).length > 0) {
      sections.push('\n=== DEPENDENCIES ===');
      sections.push(Object.entries(context.dependencies)
  ...
  private buildSystemPrompt(context: ProjectContext): string {
    return `You are an expert AI coding assistant for SwiStack, a browser-based IDE like Replit. You have deep knowledge of software development, best practices, and the specific project context.
...
- Following project conventions`;
  ...
  private buildErrorFixingPrompt(): string {
    return `You are an expert debugger. Analyze errors carefully and provide precise fixes with clear explanations. Focus on:
...
- Best practices adherence`;
  ...
  private buildCodeSuggestionsPrompt(): string {
    return `You are a code completion expert. Provide relevant, contextual suggestions that help developers write better code faster. Focus on:
  ...
  private async generateSuggestionsFromResponse(content: string, context: ProjectContext): Promise<CodeSuggestion[]> {
    ...
    codeBlocks.forEach((block, index) => {
      suggestions.push({
        ...
        code: block.code,
        filePath: '', // Would be determined by context
        confidence: 0.8,
      });
    });
